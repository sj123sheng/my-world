...
[ ] 普通闪避免疫；在脉冲剩余 `100–500ms` 时单击闪避，精准规避该次脉冲并授予 `15 秒` 洞察
...
Result (2026-07-16): BLOCKED，以上九项均未验收。目标设备 `2MN0224C12000754` 首次探测
显示 `USB Offline`，重启 hdc 后目标列表为空，无法确认设备连接和解锁，也无法安装 HAP、
执行 UITest dump、截图或读取 HUD 数值。未注入任何临时按钮坐标；移动/相机并发亦未回归。
