...
## 2026-08-16：主角 VFX 尺寸基准与档案缩放同步（修复特效放大 5 倍遮屏）

- 背景：主角模型换新资产（raw 高 0.98，旧资产约 5 倍）时档案缩放由
  0.05/3 放大 5 倍到 0.25/3，角色世界体量不变；但 `VfxSizeRatio`
  主角基准仍为 0.025/3，ratio 由定参时的 2 变 10，主角全部 ratio
  驱动特效（光柱/冲击波/符阵/火花/贴花）与刀光 `scale*2.4`、武器
  拖尾 `radiusFactor*scale` 在世界空间放大 5 倍，出现光柱/刀光遮屏。
- 决策：主角基准同步为 0.125/3 使 ratio 回到定参值 2；主角刀光倍率
  折回 0.48/0.23、拖尾 scale 乘 0.2 修正；敌方（档案缩放未变、刀光
  2.2 仍正确）不动。scale 直接驱动的接触阴影/附魔环等 08-10 后按新
  缩放调过参的项不改。
- 后续再调主角体量时必须同步：资产与档案缩放成对调整（世界体量不变
  则同步 VfxSizeRatio 基准与刀光/拖尾倍率），否则特效会按比例漂移。
- 验证：`test_asset_profile` 新增断言锁定三档 profile ratio=2；宿主
  测试集 107 项通过；OHOS arm64 `assembleHap` BUILD SUCCESSFUL。
