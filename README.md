...
HUD 的“脉冲”数值表示距下一命中点的剩余时间。观察该倒计时，在剩余 `100–500ms`
（含边界）时只点击一次“闪避”。成功后
...
- 真机验收：设备 `2MN0224C12000754` 当前离线，真机入口验收待设备恢复后补测。
  需验证 `流程`、`首领`、`推进`、`补给`、`重试` 五个新入口可见；切换 `首领` 后
  HUD 显示 Boss 阶段和 HP；触发 `重试` 后应用进程保持运行。
