...
// 按无限 ChunkCoord 生成并缓存未上传 GPU 的 Mesh 数据，供
...
class ChunkedTerrain {
 public:
  explicit ChunkedTerrain(const TerrainHeightfield& terrain,
  ...
  // 已存在的分块跳过，返回是否真正写入。
  bool storeChunk(ChunkCoord coord, TerrainChunkCpuMesh entry);
  // 卸载一批分块（升序去重）；未加载的分块忽略。
  void requestUnloads(const std::vector<ChunkCoord>& coords);
  ...
  bool hasChunk(ChunkCoord coord) const { return chunks_.count(coord) > 0; }
  const TerrainChunkCpuMesh* chunkAt(ChunkCoord coord) const;
  size_t chunkCount() const { return chunks_.size(); }
  // 当前已加载分块坐标（std::map 遍历天然升序）。
 ...
 private:
  ...
  std::map<ChunkCoord, TerrainChunkCpuMesh> chunks_;
};
