...
## 更新摘要
**所做更改**
- 新增暂停功能：setPaused方法实现暂停/恢复机制，暂停期间冻结固定步与输入消费
- 改进目标解析：新增resolveEntityPosition函数用于伤害数字精确定位
- 增强战斗事件处理：DamageNumberSystem支持按目标实体ID生成飘字效果
- 新增快照字段：冷却总时长、目标信息、状态标志等扩展GameSnapshot结构
- 优化伤害飘字系统：支持多种伤害类型和视觉效果
...
- **新增暂停功能**：暂停/恢复机制，冻结游戏逻辑但保持画面显示
- **增强的目标解析系统**：精确的目标位置解析用于伤害数字定位
- **改进的战斗事件处理**：支持多种伤害类型和视觉效果
- **扩展的快照字段**：包含冷却总时长、目标信息和状态标志
- 游戏循环初始化、启动与停止的示例性说明
...
- presentation: PerformanceGuard、VfxSystem、DamageNumberSystem
- input: InputQueue、TouchRouter
- render: Surface（含 3D 渲染状态）
- gameplay: CombatController、EncounterController
- tests: 针对 FixedStep、Loop集成和暂停功能的单元测试
...
- [damage_numbers.h:35-57](file://native/engine/presentation/damage_numbers.h#L35-L57)
...
- **DamageNumberSystem：新增的伤害飘字系统，支持多种伤害类型和精确定位**。
- InputQueue/TrouchRouter：输入队列与触摸路由，解耦输入事件分发。
- Surface：渲染表面与 3D 渲染状态载体。
- **CombatController：增强的战斗控制器，支持更多战斗状态和事件处理**。
...
## 架构总览
Loop 在独立线程中运行，以约 60fps 的频率驱动 tickOnce；每次 tickOnce 内部通过 FixedStep 将累计时间拆分为多个固定 dtMs 步，依次调用 updateFixed 完成相机、玩家、目标选择、战斗/AI、演示导演、VFX、音频与快照发布，并在 OHOS 平台进行 3D 相机更新与渲染交换。**新增暂停功能允许冻结游戏逻辑但保持画面显示**。
...
participant Pause as "暂停控制"
OS->>Thread : 创建并启动线程
loop 每帧
Thread->>Pause : 检查暂停状态
alt 已暂停
Pause-->>Thread : 跳过逻辑更新
Thread->>Render : 保持最后一帧渲染
else 未暂停
...
- 职责
  ...
  - **暂停控制**：setPaused/isPaused 方法实现暂停/恢复机制，暂停时清空排队输入避免陈旧事件。
...
- 关键成员
  ...
  - running/shouldStop/paused：原子布尔，控制主循环退出和暂停状态。
...
+damageNumbers3d : vector<DamageNumberRenderState>
}
Loop --> FixedStep : "使用"
Loop --> SnapshotStore : "发布快照"
Loop --> PerformanceGuard : "采样"
Loop --> VfxSystem : "消费/更新"
Loop --> DamageNumberSystem : "管理飘字"
...
### 暂停功能：新增的核心特性
- 实现机制
  - setPaused(bool value)：设置暂停状态，暂停时自动调用 resetInput() 清空排队输入。
  - isPaused() const：查询当前暂停状态。
  - paused 原子变量：线程安全的暂停状态管理。
- 行为特征
  - 暂停期间：冻结固定步更新和输入消费，保持最后一帧画面显示。
  - 恢复后：继续正常推进，丢弃暂停期间的陈旧输入。
  - 适用场景：暂停菜单、结算界面、加载过渡等需要冻结逻辑但保持画面的情况。

**章节来源**
- [loop.h:74-77](file://native/engine/core/loop.h#L74-L77)
- [loop.cpp:201-207](file://native/engine/core/loop.cpp#L201-L207)
- [test_loop_integration.cpp:362-380](file://tests/test_loop_integration.cpp#L362-L380)

### 目标解析系统：新增的精确位置解析
- resolveEntityPosition 函数
  - 根据实体 ID 解析世界坐标，支持玩家、训练目标、首领和敌人。
  - 返回 std::optional<Vec2>，找不到目标时返回 nullopt。
  - 用于伤害飘字的精确定位，确保数字出现在正确的实体位置。
- 支持的实体类型
  - CombatController::kPlayerId：玩家位置 (surface.player.x, surface.player.y)
  - CombatController::kTrainingTargetId：训练目标位置
  - EncounterController::kBossId：首领位置 (surface.boss3d.x, surface.boss3d.y)
  - 普通敌人：遍历 encounter.enemies 查找匹配 ID

**章节来源**
- [loop.cpp:133-152](file://native/engine/core/loop.cpp#L133-L152)
...
- 顺序与职责
  ...
  - **伤害飘字系统**：仅从本步新增的 Damage 事件生成，按目标实体定位，支持多种伤害类型。
...
U12 --> U13["damageNumbers.spawn() 按目标定位"]
U13 --> U14["更新 3D 动画/朝向/命中闪烁"]
U14 --> U15["构建并发布 GameSnapshot"]
U15 --> U16["返回"]
```
...
### DamageNumberSystem：新增的伤害飘字系统
- 功能特性
  - spawn(Vec2 position, float value, DamageNumberKind kind)：在指定位置生成伤害数字。
  - update(int64_t dtMs)：更新时间进度，移除过期飘字。
  - clear()：清空所有飘字。
  - active()：获取当前活跃的飘字列表供渲染层消费。
- 伤害类型支持
  - Normal：普通伤害（白色）
  - Heavy：大额伤害（金色，值≥15.0）
  - PlayerHit：玩家受击（红色）
- 视觉效果
  - 确定性水平散布：按序号在 [-0.018, 0.018] 内取偏移，避免重叠。
  - 缓动上浮：ease-out 曲线，先快后慢。
  - 透明度淡出：前 60% 生命保持不透明，后 40% 线性淡出。
  - 容量限制：最多同时存在 24 个飘字，超出时淘汰最旧条目。

**章节来源**
- [damage_numbers.h:35-57](file://native/engine/presentation/damage_numbers.h#L35-L57)
- [damage_numbers.cpp:1-58](file://native/engine/presentation/damage_numbers.cpp#L1-L58)
- [loop.cpp:592-623](file://native/engine/core/loop.cpp#L592-L623)

### 多线程与线程安全
- 主循环线程
  - runner 由 start 创建，stop 中 shouldStop=true 并 join 等待结束。
  - running/shouldStop/paused 为原子变量，避免数据竞争。
- 输入入队
...
### 扩展的快照字段
- GameSnapshot 新增字段
  - radianceCooldownTotalMs/currentCooldownTotalMs/corruptionCooldownTotalMs：技能冷却总时长
  - ultimateWindowMs：终结技窗口时间
  - targetPoiseBroken：目标失衡状态
  - radianceAttached/currentAttached/corruptionAttached：源附着状态
  - corroded：侵蚀状态
  - currentReaction：当前反应类型
  - pulsePhase：脉冲相位
  - 其他战斗状态字段：stamina、comboSegment、invulnerable、insightMs 等

**章节来源**
- [game_snapshot.h:7-77](file://native/engine/core/game_snapshot.h#L7-L77)
- [combat_controller.cpp:242-264](file://native/gameplay/combat/combat_controller.cpp#L242-L264)
...
- Loop 依赖
  ...
  - **DamageNumberSystem：新增的伤害飘字管理**
  - PerformanceGuard：性能采样
  - SnapshotStore：快照发布
  - Surface：渲染状态与 3D 相机
  - **CombatController：增强的战斗逻辑**
- 单向依赖
...
Loop --> TouchRouter
Loop --> VfxSystem
Loop --> DamageNumberSystem
Loop --> PerformanceGuard
Loop --> SnapshotStore
Loop --> Surface
Loop --> CombatController
Surface --> Camera3D
...
- **暂停功能性能**
  - 暂停时跳过逻辑更新，仅保持渲染，显著降低 CPU 占用。
- **伤害飘字性能**
  - 容量限制 24 个，超出时淘汰最旧条目，避免内存增长。
  - 确定性散布算法避免碰撞检测开销。
...
- 常见问题
  ...
  - **暂停功能异常**：检查 setPaused 调用是否正确，确认 paused 状态切换逻辑。
  - **伤害飘字定位错误**：验证 resolveEntityPosition 函数是否正确识别目标实体 ID。
- 定位手段
  - 启用 LOGI 日志，关注 PROFILE fps 与环境渲染指标。
  - 使用 test_fixed_step.cpp 与 test_loop_lifecycle.cpp 验证基础行为。
  - **新增测试用例**：test_loop_integration.cpp 包含暂停功能测试。
...
## 结论
Loop 作为引擎核心协调器，通过 FixedStep 实现确定性物理更新，结合严格的线程安全与性能监控，保证了稳定的帧率与可预测的游戏逻辑推进。**新增的暂停功能提供了灵活的游戏状态控制能力，支持暂停菜单和结算界面等场景**。配合 VfxSystem、AudioBridge、Surface、DamageNumberSystem 与 InputQueue，形成完整的游戏循环闭环。通过合理的固定步长上限、滑动窗口 FPS 采样与渲染降级策略，系统在移动端平台上具备良好鲁棒性与可扩展性。**改进的目标解析系统和增强的战斗事件处理进一步提升了游戏的视觉反馈和交互体验**。
...
- **暂停控制**
  - 调用 Loop::setPaused(true)：暂停游戏逻辑，清空排队输入，保持画面显示。
  - 调用 Loop::setPaused(false)：恢复游戏逻辑，继续正常推进。
