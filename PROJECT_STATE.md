...
- 敌人/NPC 模型体量调整：敌人档案缩放放大 3 倍（0.044/3→0.132/3）；
  NPC 三轮调整（3 倍→再 2 倍→嫌大缩回 2/3）累计放大 4 倍
  （0.05/3→0.20/3），体量介于主角（0.0833）与敌人（0.044）之间。
  描边世界宽度随体量同倍变化保持相对线宽；`VfxSizeRatio` 基准保持
  定参值不动，敌人 ratio 2→6，ratio 驱动特效按设计自动随模型放大。
  主角/首领不变。战斗留白（engagement_spacing）为固定世界单位未随
  体量调整，群敌视觉间距相对更挤，如需要再单独调参。
  test_asset_profile 同步锁定，宿主测试集 107 项全部通过。
- 主角特效放大 5 倍遮屏修复：08-10 换模（资产缩 1/5）与档案缩放
  放大 5 倍后世界体量不变，但 `VfxSizeRatio` 主角基准失同步使
  ratio=10，光柱/冲击波/符阵/火花与刀光/拖尾全部放大 5 倍。基准
  同步为 0.125/3（ratio 回到定参值 2），主角刀光倍率折回
  0.48/0.23、拖尾乘 0.2 修正；敌方（scale 未变）不受影响。
  test_asset_profile 锁定三档 ratio=2，宿主测试集 107 项与 OHOS
  arm64 assembleHap 通过。
...
- 出生台地—中枢回廊环境垂直切片已接入：四层原创地表图集 + 手绘控制图、
  ...
  统一环境状态，程序云、距离雾、局部椭圆湖体、岸线泡沫和轻量雨雪。
- `EnvironmentQualityProfile` 已统一控制阴影、植被、地表细节、云和水面退化；
  手动低画质与自动 `PerformanceGuard` 合并为唯一有效档。地表/植被/视觉地貌路径
  同步生成 ArkTS 资源清单，避免与 Native 常量分叉；新增环境视觉资源约 2.95 MiB，
  固定五机位和待补真机指标记录于
  `docs/environment_vertical_slice_validation.md`。
...
- 敌人附着本体染色：附着期间敌人本体基色向附着元素均色低比例呼吸
  ...
  附着渲染不参与。
- 首领转阶段失衡动画：转阶段瞬间（1→2→3，出场 0→1 除外）首领进入
  0.7s 硬直（BossPhaseBreakStaggerSeconds 纯函数锁定），发布侧强制
  受击动画并走 Hit_B 重反应变体（StaggerVariantFor 同源），与转阶段
...
- 主角技能释放本体染色：源质技能吟唱期间本体向技能元素色高比例
  ...
  闪白仍后置优先。
- 敌人死亡元素溶解染色：元素系敌人尸体淡出期间基色向原型元素色
...
- 地图/地形升级为原神式数据驱动地貌：world.json 新增 22 个地形特征
  （辉光湖盆、湖心平顶 mesa、圣所高原、灰烬劣地脊线、缓丘、回廊攀爬
  悬崖、世界边缘天际线山脊环），构建期代码生成后经 world_terrain 桥接
  注入 TerrainHeightfield 特征层；基础八度整体压缓、水面降至 -0.045，
  水域只由湖盆特征决定，地貌骨架全部来自特征层；新增 3 条主干道
  串联主路线 POI。逻辑层（地面贴合/攀爬/水域）与渲染网格采样同一
  高度场，视觉与逻辑严格一致。
- 地形渲染升级为分区生态着色：片段着色器按世界坐标矩形距离加权混合
  六区沙/草/岩配色（TerrainBiomeFor 纯函数调色板，相邻分区平滑过渡），
  主干道在地形上压出路径色带，水面升级双频涟漪 + 菲涅尔 + 高光闪点，
  世界边缘山脊环充当天际线远景并遮挡世界边界。
- 主角出生与移动视角收口：出生台地追击侦察敌已从 `world.json`
...
- 地形特征层设计经 Python 高度场镜像全量验证：22 个特征约束全部通过
  （内容点干地、坡度 < 0.55、湖心水面以下、mesa 顶精确收敛、中心区
  无杂散水域）；test_terrain_heightfield（重写，全内容点干地 + 坡度
  断言）、test_terrain_biome（新增）、test_world_layout_gen（扩展）
  通过，全量宿主测试 101 pass / 0 fail / 4 skipped（平台链路测试）。
- 地形着色器 glslangValidator 零错误；HAP assembleHap BUILD SUCCESSFUL
  （surface.cpp/shader_3d.cpp 设备侧编译链接通过）。
- 分区生态着色过渡、路径色带、水面涟漪与天际线山脊的真机观感尚未验收调优。
- 本轮出生/移动/视角收口完整宿主测试 107 项全部通过，
