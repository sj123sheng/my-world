...
// Phase 2：环境布局 blockId 校验（追加式）——assets/environment/layout.json
// 每条 placement 的 blockId 必须为 [-1, 64) 整数，-1 表示全局组。
(function checkEnvironmentBlockIds() {
  const root = path.resolve(__dirname, '../..');
  const layoutPath = path.join(root, 'assets/environment/layout.json');
  if (!fs.existsSync(layoutPath)) {
    console.error('ENVIRONMENT BLOCK CHECK FAIL: assets/environment/layout.json 不存在');
    process.exitCode = 1;
    return;
  }
  let layout;
  try {
    ...
    console.error(`ENVIRONMENT BLOCK CHECK FAIL: layout.json 解析失败: ${error.message}`);
    process.exitCode = 1;
    return;
  }
  const placements = Array.isArray(layout) ? layout : layout.placements;
  if (!Array.isArray(placements) || placements.length === 0) {
    console.error('ENVIRONMENT BLOCK CHECK FAIL: layout 缺少非空 placements');
    process.exitCode = 1;
    return;
  }
  let blockEntries = 0;
  for (const placement of placements) {
    ...
    if (!Number.isInteger(blockId) || blockId < -1 || blockId >= 64) {
      console.error(`ENVIRONMENT BLOCK CHECK FAIL: ${placement.id ?? '<unnamed>'}` +
        ` blockId 必须为 [-1, 64) 整数, 实际 ${placement.blockId}`);
      process.exitCode = 1;
    }
    if (blockId >= 0) blockEntries += 1;
  }
  if (process.exitCode !== 1) {
    console.log(`ENVIRONMENT BLOCK CHECK: placements=${placements.length}` +
      ` 全局(blockId=-1)=${placements.length - blockEntries} 区块条目=${blockEntries} 校验通过`);
  }
})();
